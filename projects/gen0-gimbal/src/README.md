...
- `app/`: Flutter application. `lib/main.dart` is the entrypoint; `lib/ble/` is the BLE client and UUID contract; `lib/control/gimbal_controller.dart` throttles jog/move commands; `lib/models/`, `lib/pages/`, `lib/widgets/`, `lib/utils/` hold state, screens, widgets, and log formatting; `test/` holds host tests. Platform folders are regenerated via `flutter create . --project-name gimbal_app` and are not committed.
