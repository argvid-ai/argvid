...
This is not a production release, validated face detector, canonical L2 protocol, or hardware safety implementation. The optional real-gimbal mode performs read-only BLE discovery and telemetry queries only; it never sends motor motion commands, and no gimbal control integration, signed release or completed hardware-in-the-loop validation is claimed. The device matrix for this revision is pending.
...
The debug APK is generated at `src/app/build/outputs/apk/debug/app-debug.apk`. Building test APKs does not execute instrumentation. On a separately authorized API29+ device, `./gradlew --no-daemon --max-workers=2 connectedDebugAndroidTest` from `src` runs instrumentation. On one approved physical device (2026-09-14) the app-package instrumentation suite — manifest-contract, scan-driven read-only F32C BLE, activity-launch diagnostics, the Compose journey and runtime recreation — executed 6/6 after enabling the manufacturer's per-app background-pop-up permission (that OEM gate had silently blocked instrumentation activity launches; see [verification](docs/verification.md)). Module test APKs run under their own package names, which that gate cannot be granted to, and one MediaStore integration test hangs on the device; both remain pending other devices. Do not interpret host tests as CameraX, codec, MediaStore, device performance, or hardware safety validation.
...
Camera and microphone are the runtime permissions requested by the capture flow. The opt-in real-gimbal discovery mode additionally requests `BLUETOOTH_SCAN` (declared `neverForLocation`) and `BLUETOOTH_CONNECT` on API 31+, with legacy `BLUETOOTH` and `ACCESS_FINE_LOCATION` (both `maxSdkVersion="30"`) declared for API 29–30 scanning; these are granted before any BLE operation and power read-only discovery only. Library manifests also declare the normal WAKE_LOCK permission and an app-scoped signature permission; there is no INTERNET or network-state permission. The app opts out of Android backup and excludes its private data from device transfer. This does not control independent gallery/photo-backup apps: saved MediaStore clips can be handled by those apps under the user's device settings. Camera frames, microphone audio and saved clips may contain sensitive surroundings: the entrypoint supplies no automatic face masks or voice anonymization, and its camera preview is not anonymized. The mask-processing unit tests only exercise explicitly supplied rectangles. Do not use real personal media as repository fixtures.

The simulator models capabilities, acknowledgments, ranges, motion, and stop semantics in process and remains the default gimbal source: the Session surface labels its state as simulated unless the user explicitly selects the real-gimbal (BLE) source. A [Gimbal](src/feature/session/src/main/kotlin/ai/argvid/gen0/session/GimbalConsoleScreen.kt) tab hosts the gimbal console and drives whichever source the session selected: the simulator, or the opt-in real F32C BLE bridge ([F32cBleGimbalLink](src/adapter/gimbal/src/main/kotlin/ai/argvid/gen0/gimbal/F32cBleGimbalLink.kt)). The bridge issues real motor commands (position moves, hold/emergency stop) only through that explicit selection; its command receipts mean transport acceptance, measured angles come only from read-only queries, and relative nudges are refused while measured telemetry is stale. There is no cloud sync, network provider, reshoot, preset, or decision-model path. Failed-save retry/abandon is available within the running session; there is no background cleanup job or process-death recovery service.
